function updateLanguageInfo(language) {
    ...
    if (language === "minion") {
        info.textContent = `Minion Language (Minionese)
The language spoken by the Minions in "Despicable Me" is known informally as "Minionese." Unlike fully constructed languages, Minionese is a playful hybrid of multiple real-world languages, including English, Spanish, French, Italian, and even bits of Korean or Japanese.
...
Minionese works because it taps into linguistic familiarity without requiring comprehension. Viewers can often guess the meaning based on similarity to known words, making it a highly effective comedic tool rather than a formal language.`;
        ...
        info.textContent = `Game of Thrones Languages (Dothraki & Valyrian)

In Game of Thrones, languages such as Dothraki and High Valyrian are among the most fully developed fictional languages in modern media. Originally, author George R. R. Martin created only a few words, but for the TV adaptation, linguist David J. Peterson expanded them into complete systems.
...
These languages work as true constructed languages (conlangs), with defined grammar, vocabulary, and phonology. Unlike Star Wars or Minionese, they can be learned and used for real communication.`;
    } 
    else if (language === "pirate") {
        info.textContent = `Pirate Speak

"Pirate speak" is not a real historical language but a stylized dialect of English shaped by popular culture. Its modern form comes largely from portrayals of pirates in films and literature, especially the influence of actor Robert Newton in the 1950 film Treasure Island.
